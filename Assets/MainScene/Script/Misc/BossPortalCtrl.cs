...
public class BossPortalCtrl : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            timer = 0.3f;
            FloatText.SetActive(Hit.Overlap(sensor, LayerMask.GetMask("Player"), c) > 0 && Chcek());
        }
        if (FloatText.activeSelf && Input.GetKeyDown(KeyCode.F))
        {
            if (!tping)
            {
                if (Hit.Overlap(sensor, LayerMask.GetMask("Player"), c) > 0 && Chcek())
                {
    ...
    private bool Chcek()
    {
        return player.getBool("hasItem_玩具车") && player.getBool("hasItem_玩具鸭子") && player.getBool("hasItem_皮球") && player.getBool("hasItem_飞机");
    }
