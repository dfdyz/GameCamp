...
public class Mob4Ctrl : MonoBehaviour
{
    [Header("Instance")]
    [SerializeField]
    ...
    [SerializeField]
    private bool EnableShootOffest = true;
    [SerializeField]
    private float OffestAngLimit = 10f;
    ...
    void Update()
    {
        ...
        if (ScanTimer <= 0f && stun <= 0f)
        {
            ...
            {
                targetpos = c[0].gameObject.transform.position;
                Vector3 p = gameObject.transform.position;
                float tmp = (targetpos - p).x;
                vel.x = MoveSpeed*(tmp>0 ? 1f:-1f);
                hasTarget = true;
            }
            else
            {
        ...
        if(hasTarget)
        {
            float tmp = (targetpos - gameObject.transform.position).x;
            if(Mathf.Abs(tmp) <= 0.3f)
            {
                vel.x = 0;
            }
        }

        if (hasTarget && stun <= 0f && attackCoolDownTimer <= 0f && Hit.Overlap(AtkSensor, lm, c) > 0)
        {
            ...
            {
                Atk(rch.collider.gameObject.transform.position, EnableShootOffest);
            }
            else if (rch2 && rch2.collider && rch2.collider.gameObject.layer == LayerMask.NameToLayer("Player"))
            {
                Atk(rch2.collider.gameObject.transform.position, EnableShootOffest);
            }
            else if (rch3 && rch3.collider && rch3.collider.gameObject.layer == LayerMask.NameToLayer("Player"))
            {
                Atk(rch3.collider.gameObject.transform.position, EnableShootOffest);
            }
        }
    ...
    private void Atk(Vector3 target,bool isOff)
    {
        Vector3 p = gameObject.transform.position;
        Vector3 v = target - p;
        if (isOff)
        {
            float ang = Random.Range(-OffestAngLimit, OffestAngLimit);
            v = Quaternion.AngleAxis(ang, Vector3.forward) * v;
        }
        p.z = 1f;
        v.z = 0f;
